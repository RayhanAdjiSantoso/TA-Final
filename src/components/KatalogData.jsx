...
const KatalogData = ({ activeTabProp }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (databases.length > 0 && !selectedDatabase) {
          // PERBAIKAN: Gunakan id_database (integer)
  ...
  const handleDatabaseCheckbox = (dbId) => {
  ...
  const handleUpload = async () => {
    ...
    const selectedDbConfigs = availableDatabases
      .filter(db => selectedDatabasesForUpload.includes(db.id_database))
      .map(db => ({
        id_database: db.id_database,
        nama_database: db.nama_database, // Gunakan nama_database (dari kolom DB)
  ...
  const handleDeleteDatabase = async (dbId, dbName) => { // Parameter dbId adalah integer
    if (!window.confirm(`Apakah Anda yakin ingin menghapus database "${dbName}"?`)) {
      return;
    }

    try {
      const response = await fetch(`http://localhost:5002/api/database/${dbId}`, {
        method: 'DELETE',
      });
      ...
      if (result.success) {
        ...
        if (selectedDatabase === dbId && availableDatabases.length > 1) {
          const remainingDbs = availableDatabases.filter(db => db.id_database !== dbId);
          if (remainingDbs.length > 0) {
            setSelectedDatabase(remainingDbs[0].id_database);
          }
        }
                          ...
                          {availableDatabases.map((db, index) => (
                            <div key={index} style={{ marginBottom: '8px' }}>
                              <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                                <input
                                  type="checkbox"
                                  checked={selectedDatabasesForUpload.includes(db.id_database)}
                                  onChange={() => handleDatabaseCheckbox(db.id_database)}
                              ...
                              {availableDatabases.map((db, index) => (
                                <tr key={index}>
                                  ...
                                  <td style={{ textAlign: 'center' }}>
                                    <button
                                      onClick={() => handleDeleteDatabase(db.id_database, db.name)}
                ...
                <div className="database-viewing-section" style={{ marginBottom: '20px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', marginBottom: '15px' }}>
                    ...
                    <select
                      ...
                      onChange={(e) => setSelectedDatabase(parseInt(e.target.value))}
                      style={{
                        padding: '8px 12px',
                        borderRadius: '4px',
                        border: '1px solid #ddd',
