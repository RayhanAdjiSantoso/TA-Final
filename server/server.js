...
app.delete('/api/database/:id', async (req, res) => {
  ...
  try {
    ...
    const checkVisualisasi = await systemPool.query(
      'SELECT COUNT(*) as count FROM visualisasi WHERE penyimpanan_database = $1',
      [dbId]
    );
    ...
    const checkAnalisis = await systemPool.query(
      'SELECT COUNT(*) as count FROM analisis WHERE penyimpanan_database = $1',
      [dbId]
    );
  ...
  try {
    ...
    if (targetDbId) {
      ...
      if (dbResult.rows.length > 0) {
        ...
        isMySQL = dbResult.rows[0].jenis === 'mysql';
        console.log(`Using database: ${dbResult.rows[0].label} (${dbResult.rows[0].jenis})`);
      }
    }
    ...
    if (params && Object.keys(params).length > 0) {
      ...
      // PERBAIKAN: Replace parameter dengan placeholder sesuai database type
      for (const [paramName, index] of Object.entries(paramMap)) {
        const replaceRegex = new RegExp(`:${paramName}\\b`, 'g');
        if (isMySQL) {
          // MySQL menggunakan ? sebagai placeholder
          modifiedQuery = modifiedQuery.replace(replaceRegex, '?');
        } else {
...
app.post('/api/visualizations', async (req, res) => {
  ...
  try {
    ...
    const penyimpananDatabase = parseInt(visualisasi.database);
    
    console.log('=== SAVING VISUALIZATION ===');
    console.log('Database ID:', penyimpananDatabase);
    console.log('Type:', typeof penyimpananDatabase);
    ...
    const result = await systemPool.query(
      ...
      [
        ...
        parameterQuery,
        chartDataString,
        penyimpananDatabase // ← Langsung gunakan integer
      ]
    );
...
app.get('/api/database/:dbId/visualisasi', async (req, res) => {
  ...
  try {
    console.log('Fetching visualisasi for database ID:', dbId);
    ...
    const result = await systemPool.query(
      `SELECT * FROM visualisasi 
       ...
       ORDER BY created_at DESC`,
      [databaseId] // ← Integer, bukan string
    );
...
app.post('/api/analisis', async (req, res) => {
  ...
  try {
    // PENTING: Gunakan id_database langsung (integer)
    const penyimpananDatabase = parseInt(database);
    
    console.log('=== SAVING ANALISIS ===');
    console.log('Database ID:', penyimpananDatabase);
    console.log('Type:', typeof penyimpananDatabase);
...
app.get('/api/database/:dbId/analisis-with-visualisasi', async (req, res) => {
  ...
  try {
    ...
    // LANGSUNG gunakan dbId sebagai integer
    const databaseId = parseInt(dbId);
