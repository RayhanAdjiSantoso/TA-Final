...
// Path file untuk menyimpan konfigurasi database
const DB_CONFIG_FILE = path.join(__dirname, 'databases.json');

// Fungsi untuk membaca konfigurasi database dari file
const readDatabaseConfig = () => {
  ...
  // Jika file tidak ada atau error, return database default
  return [
    {
...
// Fungsi untuk menyimpan konfigurasi database ke file
const saveDatabaseConfig = (databases) => {
  try {
    fs.writeFileSync(DB_CONFIG_FILE, JSON.stringify(databases, null, 2));
    return true;
  } catch (error) {
...
// Konfigurasi koneksi PostgreSQL default (untuk sistem)
const pool = new Pool({
  user: process.env.DB_USER || 'rayhanadjisantoso',
...
// Verifikasi koneksi database default
...
// Endpoint untuk mendapatkan semua tabel (legacy)
...
app.post('/api/database/test-connection', async (req, res) => {
  ...
  // Tentukan host dan port berdasarkan tipe database
...
app.post('/api/database/add', async (req, res) => {
  ...
  try {
    // Baca konfigurasi database yang ada
    const databases = readDatabaseConfig();
    
    // Generate value unik untuk database
    const value = `${type}_${database}_${Date.now()}`;
    
    // Tentukan host dan port berdasarkan tipe
    const host = 'localhost';
    const port = type === 'postgresql' ? 5432 : 3306;
    
    // Cek apakah nama database sudah ada
    const nameExists = databases.some(db => db.name === name);
    if (nameExists) {
      return res.status(400).json({ 
        success: false, 
    ...
    // Simpan ke file
    const saved = saveDatabaseConfig(databases);
...
app.delete('/api/database/:value', async (req, res) => {
  ...
  try {
    ...
    // Cari database yang akan dihapus
    const dbIndex = databases.findIndex(db => db.value === value);
    ...
    // Hapus database dari array
    const deletedDb = databases.splice(dbIndex, 1)[0];
...
app.get('/api/database/:dbValue/table/:table/count', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const count = dbConfig.type === 'mysql' ? result[0][0].count : result.rows[0].count;
...
app.get('/api/database/:dbValue/data/:table', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const data = dbConfig.type === 'mysql' ? result[0] : result.rows;
